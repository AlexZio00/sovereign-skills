...
> **v6.5.8 변경사항:** 정비 릴리즈 — 스킬 추가/제거 없음, v6.5.7 이후 내부 포크의 변경분을 선별 이식(그 기간 동안 업스트림에서 바뀐 20개 중 6개 + 손수 대조한 `code-autopsy` 이식 1건 포함). `doc-drift`(신규 Derivability 신호 — 코드로 기계적 재구성 가능한 사실을 하드코딩한 줄은 현재 값이 맞더라도 구조적으로 drift 위험이 높다고 플래그), `integration-intake`(신규 Analogy-trap 체크 — "X가 이렇게 하니까 우리도"류 가치 주장은 3가지 질문에 답해야만 근거로 인정), `session-start`(신규 질의-조건부 로드 규칙 — 온디맨드 메모리 파일은 대화가 실제로 그 주제를 다루는지 확인한 후에만 로드, 키워드 겹침만으로는 로드 안 함), `goal-lock`(스킬/훅/게이트 자체 신뢰도를 측정하는 태스크용 신규 선택 필드 EVAL TYPE, Silent Self-Correction 안티패턴, 수정 전 원본 DONE EVIDENCE 실행결과를 기록하는 First-Attempt Ledger, 예산 오판으로 인한 조기 태스크 포기를 막는 Early Self-Doubt Boundary 노트), `pre-push`(code-reviewer 서브에이전트 불가 시 완전 스킵 대신 인라인 축약 리뷰로 대체, 신규 Step 3.5 Public-Mirror Scrub — 비공개 소스의 공개 미러 리포용 push-time WARN-only 백스톱, 신규 opt-in 고위험 diff gap-sweep 2차 패스와 opt-in 다각도 병렬 재공격, Fix loop에 스킵사유 로깅 추가), `session-checkpoint`(신규 Stage 1 CT 승격 큐잉 — `scripts/ct_promotion_queue.py`, 마커 파일 기반 opt-in, LLM 판단 없는 순수 결정론 토큰겹침 클러스터링이며 MEMORY.md/context-log.md 자체엔 쓰지 않음, 35케이스 회귀 스위트 동봉, invocation-log 스킵 조건이 Triple Gate의 활동량 조건만 재사용하고 다른 자동트리거용 24시간 조건은 의도적으로 제외한다고 명확화), `code-autopsy`(코드스멜 용어 7종 추가 + 의존성방향 위반의 SOLID 원칙 명시, wrapper/proxy forwarding 정확성 체크, 근거인용 전용(추론된 "취지" 금지) governing-rules 위반 체크, Q3에 off-by-one/falsy-zero/copy-paste/미이스케이프 정규식 + Python mutable-default-argument·late-binding-closure 함정 추가, Q7 메모리누수 체크에 클로저-캡처-대형객체 패턴 명시, STEP 0에 함수단위 계약 체크 + 명시적 diff 스코프 고정 + governing CLAUDE.md/rules 탐색 추가, 삭제/치환된 모든 라인에 대한 신규 재확립-불변식 체크, 풀파이프라인과 Quick Mode 사이의 신규 [FAST MODE] `--fast` 티어).
>
> **v6.5.7 변경사항:** 정비 릴리즈 — 스킬 추가/제거 없음, 여러 스킬이 LLM 자체채점 대신 실동작하는 결정론 스크립트를 갖추게 됨. `project-overview`(`generate_overview.py`가 더 이상 스텁이 아님 — 레지스트리 파싱, state-snapshot 추출, AUTO-block 렌더/교체를 완전 구현, 비신뢰 입력 테이블 셀 이스케이프 + malformed-marker 복구 포함, 유닛+통합 테스트로 뒷받침), `scope`(Quick/Full 모호성 게이팅과 BRIEF.md 최소항목 검증이 이제 이식된 `ambiguity_gate.py`를 경유 — 볼드텍스트를 헤더로 오카운트하는 버그를 고정하는 회귀 테스트 포함), `skill-ops` → v1.2(Health Mode 버킷 분류와 Quality Mode S/U/S_Q 점수 산출이 수동 산술 대신 `skill_health_bucket.py`를 경유), `collab-audit`(Step 0.6 소스 위생 필터링이 이제 `session_hygiene_scan.py`를 경유), `session-checkpoint`(인덱스 백링크 없는 메모리 쓰기를 플래그하는 신규 Discoverability Check 단계, 타임아웃/kill 부분출력 가드, Reflexion 교훈 품질 게이트 2종, raw observation용 PII 마스킹 규칙, Key Files 검증을 `validate_memory_claims.py` 스크립트로 이관), `goal-lock`(스코프 체크 표면을 건드린 파일뿐 아니라 인터페이스/API 변경까지 확대, 벤치마크 근거 체인길이 Dominant Variable 노트, Stop-hook 순서 게이트를 검증된 4조건 구현으로 업그레이드, 신규 Safety Layers 섹션, enum형 VERIFY 실패 라벨, REFINE의 DELTA CHECK에 자기판단 주의문, 백그라운드 태스크 종료 핸드셰이크), `code-autopsy`(리뷰어의 흔한 정당화 8종 + 반박을 담은 Rationalization Table, 암산 대신 스크립트 기반 심각도 산술을 권장하는 노트), `eval-leakage-audit`(17→18패턴 taxonomy — 자가개선 루프 내 Goodhart 공진화 추가 — + Reviewer Independence 정직 4-라벨 체크), `integration-intake`(읽기전용 `tools:` frontmatter, 리다이렉트형 `not_for` 항목, 단계별 고정 오판 enum 라벨 7종), `pre-push`(명시적 `depends_on`/`concurrency_profile` frontmatter, 읽기전용 git 명령과 게이트된 `git push`를 구분하는 Autonomy Boundary 노트, 외부 보안 카탈로그 참조 링크), `session-start`(`depends_on`/`concurrency_profile` frontmatter, Phase 2.2-2.4를 고정 stdout 계약을 가진 결정론 명령으로 재작성, 신규 번호 없는 L0 Inheritance 섹션), `doc-drift`(Risky/Ambiguous 카테고리를 보조 근거로만 공급하는 신규 Step 0 결정론 사전필터), 그리고 `full-audit`/`project-check`/`project-init`/`freeze`/`stepback`/`next-action`/`clean-room` 전반에 `depends_on`/`concurrency_profile` frontmatter 채택 — 이 중 여럿은 도구 카테고리 태그가 붙은 Scope Boundary 테이블도 얻음. **이번 릴리즈에서 드러난 알려진 갭**: `project-check`(및 일부 형제 스킬)가 Safety Layers/Error Recovery 헤더에 번호 없는 "상속 원칙" 노트를 얻었는데 — 과거 릴리즈에서 10개 스킬 전체의 동일 헤더에서 번호 붙은 `(L0 §N)` 인용을 의도적으로 제거했었다. 이번 버전의 노트는 제거된 인용이 아니라 번호 없는/일반형 노트지만, 헤더-주석 패턴 자체는 돌아온 것 — 다음 릴리즈가 이 위에 더 쌓기 전에 의식적 판단이 필요.
>
> **v6.5.6 변경사항:** 정비 릴리즈 — 스킬 추가/제거 없음. `eval-leakage-audit`(13→17패턴 taxonomy — respawn masking, pseudo-replication, stimulus-calibration gap, 미감사 비용절감 스킵 추가), `goal-lock`(실행 증거가 명시 지시와 모순될 때 구현을 강행하지 못하게 막는 S7 정지 규칙, 5단계 evidence-rigor 사다리 + 실패우선 보고순서 + 금지 헤지 문구, "layer laundering" 성공위장 패턴, evidence-rigor 사전명세 노트), `full-audit`(개별로는 기각됐어도 한 영역에 UNCERTAIN/NIT 발견이 쌓이면 death-by-thousand-cuts 위험을 플래그하는 composite-accumulation-gate, 미검증 가정에 기대는 CONFIRMED 판정용 Assumption Ledger), `session-checkpoint`(선택적 `regime`/`escalate_if` 교훈 필드, first-attempt-pass/rework/resolvedBy를 추적하는 선택적 `outcomes` 필드, 교훈 아카이빙 OR조건 2번째 추가 + 아카이빙 전 필수 상호참조 확인), `code-autopsy` → v7.2(Q1용 code-smell 어휘, 구체적 실패시나리오 요구, Q5 하 객체단위 authorization, Q7 하 네트워크/DB/스트리밍/캐시 체크 확장, Q8 하 shallow-module + ADR충돌 체크, 수치형 신뢰도 임계 시스템, 동점 비교시 outcome-ceiling→process-metric 전환), `pre-push` → v3.8.0(기존 `scan_secrets.pl`과 나란히 `scan_secrets.py` 탑재 — Python 존재 시 우선, Perl은 폴백 — + upstream 저작권 표시용 `LICENSE.txt`; 참고: 이번 릴리즈에서 두 스캐너의 패턴 커버리지가 서로 다름이 발견됨 — 예: Python 포트는 Perl에 없는 f13 Slack-webhook 체크를 추가했고, 기존 패턴 여럿도 양쪽에서 변형 커버 범위가 더 좁거나 넓음 — 아직 조정 안 된 알려진 갭으로 기록).
>
> **v6.5.5 변경사항:** 정비 릴리즈 — 스킬 추가/제거 없음. `eval-leakage-audit`(8→13패턴 taxonomy — dual-fail-flag, asymmetric-baseline self-falsification, evidence-burn, ungraded-grader 4-gate 체크, ceiling-task 탐지 — + stratification-substantiation 체크리스트, 정직한 reviewer-independence 4-라벨 판정), `goal-lock`(마지막 편집 이후 검증이 안 돌았으면 완료를 막는 순서 게이트, exit-code 아닌 산출물용 evidence-channel 분기, comprehension 체크, 신규 성공위장 가드 3종), `pre-push` → v3.7(cross-bundle 합동 패스 + 3상태 오탐 게이트), `integration-intake`(headroom 사전분류, trait-vs-procedure 종료조건, 효과성 주장 삼중확인), `session-start`(모델ID allowlist에 `claude-sonnet-5` 추가 — 거짓 "invalid model" 경고 수정), `setup`(중복된 `/setup` 트리거 제거), `full-audit`(rule-dry-run 검증 레이어), `clean-room`(upstream autobahn v0.14.0과 조정 — 독립 재스윕에 N=1 상한), `code-autopsy`(Q10 oracle-redefinition 탐지), 그 외 `session-checkpoint`·`skill-ops`·`collab-audit`·`scope`에 소소한 정비.
>
...
| [eval-leakage-audit](../eval-leakage-audit/) | **신규.** eval/metric/holdout이 실제로 독립된 외부 그라운드트루스를 확보하는지, 아니면 순환 자기확인인지 18-패턴 taxonomy로 감사. 읽기 전용 |
