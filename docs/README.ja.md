...
> **v6.5.12の変更点:** リファインメントリリース — スキルの追加・削除なし。20スキルのうち5スキルへの差分移植。`integration-intake`に利益相反チェック、プラグインのライフサイクルフックおよびMCPツール説明のインジェクション点検、主張ごとのCapability Mapを備えた7段階デプロイパイプライン(DRAFTED → OBSERVED)を追加。`session-checkpoint`は未決の判断と因果的結論に検証根拠`basis`(`run|doc|dialogue`)を付与。`doc-drift`にCLIインターフェース同期シグナルと不変条項の侵食シグナルの2つを追加。`goal-lock`は根本原因が不確実と宣言する前に最も安価な判別チェックを1回実行(S5)し、繰り返すブロッカーが実行エラーか入力シートの設計エラーかを切り分ける(S6)。`full-audit` → v1.3はレビュアー1回あたりのバッチサイズ上限を追加。過去のリリースノートは[CHANGELOG.md](../CHANGELOG.md)に移動しました。
