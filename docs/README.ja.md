[English](../README.md) | [한국어](README.ko.md) | 🌐 **日本語** | [中文](README.zh.md) | [Español](README.es.md)

# sovereign-skills v6.5.7

Claude Codeプロジェクトのライフサイクル全体をカバーする20個のスキル — セットアップから日常ワークフロー、コードレビュー、セッション管理、ガバナンスまで。各スキルは単体で使用可能で、全シーケンスで全工程をカバーします。

> **v6.5.7の変更点:** リファインメントリリース — スキルの追加・削除なし。複数のスキルでLLMの自己採点に代わる決定論的スクリプトが稼働するようになった。`project-overview`（`generate_overview.py`がスタブでなくなり、レジストリのパース・state-snapshot抽出・AUTOブロックのレンダー/置換を完全実装、信頼できない入力に対するテーブルセルのエスケープと不正マーカーからの復旧を追加、ユニット+統合テストで裏付け）、`scope`（Quick/Fullの曖昧性ゲーティングとBRIEF.mdの最小項目バリデーションが移植版`ambiguity_gate.py`経由になり、太字テキストを見出しとして誤カウントするバグを固定するリグレッションテストを追加）、`skill-ops` → v1.2（Health Modeのバケット分類とQuality ModeのS/U/S_Qスコアリングが手計算から`skill_health_bucket.py`経由に）、`collab-audit`（Step 0.6のソース衛生フィルタリングが`session_hygiene_scan.py`経由に）、`session-checkpoint`（インデックスへの逆リンクがないメモリ書き込みを検出する新規Discoverability Checkステップ、タイムアウト/kill時の部分出力ガード、Reflexionレッスン品質ゲート2種、生の観察に対するPII削除ルール、Key Files検証を`validate_memory_claims.py`スクリプトへ移行）、`goal-lock`（スコープチェックの対象を変更ファイルだけでなくinterface/API変更にも拡大、ベンチマーク裏付けのチェーン長Dominant Variableノート、Stop-hookのオーダーゲートを検証済み4条件実装にアップグレード、新規Safety Layersセクション、enum形式のVERIFY失敗ラベル、REFINEのDELTA CHECKに対する自己判定の注意書き、バックグラウンドタスク終了ハンドシェイクを追加）、`code-autopsy`（レビュアーがよく使う正当化8パターン+反論のRationalization Table、暗算よりスクリプト計算での深刻度算出を推奨するノートを追加）、`eval-leakage-audit`（17→18パターンのtaxonomy — 自己改善ループにおけるGoodhart共進化を追加 — Reviewer Independence Honest 4-Labelチェックも追加）、`integration-intake`（読み取り専用の`tools:` frontmatter、リダイレクト形式の`not_for`エントリ、フェーズごとに固定された誤判定enumラベル7種）、`pre-push`（明示的な`depends_on`/`concurrency_profile` frontmatter、読み取り専用のgitコマンドとゲート対象の`git push`を区別するAutonomy Boundaryノート、外部セキュリティカタログへの参照リンク）、`session-start`（`depends_on`/`concurrency_profile` frontmatter、Phase 2.2-2.4を固定stdout契約付きの決定論的コマンドに書き換え、新規の番号なしL0 Inheritanceセクション）、`doc-drift`（Risky/Ambiguousカテゴリを裏付け証拠専用として供給する新規Step 0決定論的プレフィルター）、そして`full-audit`・`project-check`・`project-init`・`freeze`・`stepback`・`next-action`・`clean-room`全体に`depends_on`/`concurrency_profile` frontmatterを採用（うち数スキルはツールカテゴリでタグ付けされたScope Boundaryテーブルも追加）。**このリリースで判明した既知のギャップ**：`project-check`（および数スキル）がSafety Layers/Error Recoveryの見出しに番号なしの「継承原則」ノートを追加した — 過去のリリースでは全10スキルの同じ見出しから番号付き`(L0 §N)`引用を意図的に取り除いていたが、今回のノートは削除された引用ではなく番号なし/汎用な形で復活しており、見出しへの注記パターン自体は戻ってきている。次のリリースでこの上にさらに積み上げる前に、意識的な判断が必要。
>
> **v6.5.6の変更点:** リファインメントリリース — スキルの追加・削除なし。`eval-leakage-audit`（13→17パターンのtaxonomy — respawnマスキング、疑似反復（pseudo-replication）、刺激校正ギャップ、未監査のコスト削減スキップを追加）、`goal-lock`（実行時の証拠が明示的な指示と矛盾する場合に実装の押し通しをブロックするS7 stopルール、5段階のevidence-rigorラダー+失敗優先の報告順序+禁止ヘッジ表現、「レイヤーロンダリング」成功偽装パターン、evidence-rigorのpre-specノートを追加）、`full-audit`（個別には却下されたにもかかわらずUNCERTAIN/NITの発見が1領域に積み重なる"死の千枚切り"リスクを検知するcomposite-accumulation-gate、未検証の前提に依存するCONFIRMED判定用のAssumption Ledgerを追加）、`session-checkpoint`（オプションの`regime`/`escalate_if`レッスンフィールド、初回成功/やり直し/解決者を追跡するオプションの`outcomes`フィールド、レッスンアーカイブ用の2つ目のOR条件+アーカイブ前の必須クロスリファレンスチェックを追加）、`code-autopsy` → v7.2（Q1向けのcode-smell語彙、具体的な失敗シナリオの要件、Q5でのオブジェクトレベル認可、Q7でのネットワーク/DB/ストリーミング/キャッシュチェック拡張、Q8での浅いモジュール+ADR矛盾チェック、数値化された確信度閾値システム、同点比較時のoutcome-ceilingからprocess-metricへの切り替えを追加）、`pre-push` → v3.8.0（既存の`scan_secrets.pl`に加えて`scan_secrets.py`を同梱 — Pythonが存在すればそちらを優先、Perlはフォールバック — 上流帰属表示用の`LICENSE.txt`も追加。注：今回のリリースで2つのスキャナーのパターンカバレッジに差異があることが判明 — 例えばPython版にはPerl版にないf13 Slack-webhookチェックが追加されている一方、既存の複数パターンは双方でカバーするバリアントの範囲が狭かったり広かったりする — 未解消の既知のギャップとして記録)。
>
> **v6.5.5の変更点:** リファインメントリリース — スキルの追加・削除なし。`eval-leakage-audit`（8→13パターンのtaxonomy — dual-fail-flag、非対称ベースライン自己反証、evidence-burn、無評価グレーダーの4-gateチェック、ceiling-task検出を追加 — 加えて層化根拠チェックリストと正直なreviewer-independence 4-labelの判定を追加）、`goal-lock`（最後の編集後に検証が一度も走っていない場合に完了をブロックするorderゲート、exit-codeを返さない成果物向けのevidence-channel分岐、comprehensionチェック、新規の成功偽装ガード3種を追加）、`pre-push` → v3.7（クロスバンドル合同パス+3状態の偽陽性ゲート）、`integration-intake`（headroom事前分類、trait-vs-procedure終了判定、有効性クレームの三重チェック）、`session-start`（model-IDのallowlistに`claude-sonnet-5`を追加 — 誤った「invalid model」警告を修正）、`setup`（重複した`/setup`トリガーを削除）、`full-audit`（rule-dry-run検証レイヤーを追加）、`clean-room`（上流のautobahn v0.14.0と整合 — 独立再スイープにN=1上限を追加）、`code-autopsy`（Q10のoracle再定義検出を追加）、その他`session-checkpoint`・`skill-ops`・`collab-audit`・`scope`にも小さな改善。
>
> **v6.5の変更点:** 新規：`eval-leakage-audit`（eval/metric/holdoutが実際に独立した外部グラウンドトゥルースを確保しているか、それとも循環的な自己確認かを8パターンのtaxonomyで監査 — 読み取り専用）、`doc-drift`（Claude Codeがコンテキストに読み込むメモリ/ドキュメント — CLAUDE.md/MEMORY.md/skills/agents/commands — を、古い主張・相互矛盾・危険/曖昧な表現の3種で監査し、優先度付き修正リストを生成）。更新：`project-init`（大文字小文字を区別するファイルシステムでスキル読み込みを失敗させ得る`skill.md`→`SKILL.md`の命名ミスを修正 + Phase 3テンプレートを`references/templates.md`に外部化）、`pre-push` → v3.6（シークレットスキャナーパターン2種追加 — f11 diff内のプロンプトインジェクション文字列、f12 non-PyPIサプライチェーンインデックスURL — + Step 0フックパイプラインヘルスチェック）、`scope`（Mid-Task Scope Drift 10倍発見ルール追加）、`collab-audit`（Step 0.6ソース衛生フィルター追加 — 自動派生のサブエージェント/スレッドセッションを有機的なユーザーセッションと誤認しないよう除外）、`full-audit`/`integration-intake`（両方にSafety Layersセクション追加。`integration-intake`はさらにPhase 1.8 M軸サーフェス選択ステップを追加 — パターンがどのサーフェス（プロンプト/ルール/フック/スキル）に置かれるべきかをルーティング前に判断）、`goal-lock`（`migration`タスクテンプレート追加）、`project-overview`（Rationalization Table追加）、`stepback`（Dominant Variableセクション + frontmatterフィールド追加）。
>
> **v6.4の変更点:** 新規：`full-audit`（領域全体の網羅的監査 — 決定論的スイープ+内容レビュー、永続カバレッジマップ、偽陽性防止kill-test）、`integration-intake`（外部スキル/エージェント/ルール/プラグイン導入の5項目スクリーニングゲート、provenance/インジェクションチェック付き）、`clean-room`（安全隣接リクエストを安全スコープに切り出し、完全に隔離されたfresh-contextサブエージェントが実行 — LilMGenius/paperthinの「autobahn」スキルをMITライセンスの下で適応、ファイルシステム隔離+ledger記録タイミングのアップグレード付き）。更新：`goal-lock`（長時間タスクのチェックポイントごとにCONSTRAINTS/SCOPE-Excludeを再確認・再出力）、`session-checkpoint`（新規Attestationフェーズ — `handoff_attestation.py`同梱のevidence-chain receiptログ、次セッションのSessionStartフックによるハンドオフ改ざん検知用）。
>
> **v6.3の変更点:** 新規：`skill-ops`（スナップショット/ロールバック + 使用状況 + 呼び出し追跡ハブ）、`next-action`（ハンドオフ/git/lessons/STATEを読み込み、影響度順に次のアクション上位3件を提案）、`project-overview`（決定論的クロスプロジェクト状況マップ）。`code-autopsy` → v7.1（質問ごとのサブチェック強化）、`pre-push` → v3.5（サプライチェーンIOC 9パターン）、`goal-lock`/`session-checkpoint`/`session-start`/`scope`/`stepback`/`freeze` 全て強化。既存12スキル全てに `not_for`/`see_also` frontmatterを追加し発見性を改善。

---

## クイックスタート

**新規プロジェクト（15分）：**
```
/project-init       →  CLAUDE.md + ROADMAP + .gitignore + .env.example
/setup              →  rules/ + hooks + memory/ + エージェントルーティング + チーム
日常：
  /session-start      セッション開始時
  /scope              機能実装前（IN/OUT/終了基準を定義）
  /freeze             実装前（編集可能領域を宣言）
  /goal-lock          目標をロック、PLAN→DO→VERIFYループを強制
  /stepback           いつでも — 方向確認、10行
  /next-action        いつでも — 現在の状態を読み、次のアクション上位3件を提案
  /code-autopsy       12Qコードレビュー + 重要度スコア + デプロイ判定
  /pre-push           push前（シークレットスキャン + AIレビュー）
  /session-checkpoint セッション終了時
```

**既存プロジェクト（5分）：**
```
/project-check      →  4次元スコア + 深刻度別ギャップリスト
/code-autopsy       →  12Qコードレビュー（あらゆるLLMでスタンドアロンプロンプトとして使用可能）
/collab-audit       →  14セクションAIコラボレーション診断
```

**ガバナンス（必要時）：**
```
/integration-intake →  外部スキル/エージェント/ルール/プラグイン導入前 — 5項目スクリーニングゲート
/full-audit         →  領域全体（コードベース/ドキュメント/スキル/メモリ/設定）の網羅監査 + カバレッジマップ
/clean-room         →  タスクに安全隣接要素と安全な作業が混在する時
/eval-leakage-audit →  eval/metric/holdoutを信頼する前 — 循環的自己確認でないか点検
/doc-drift          →  読み込まれたコンテキスト（CLAUDE.md/MEMORY.md/skills）を古い/矛盾した表現で監査
```

---

## スキル一覧

### セットアップフェーズ

| スキル | 機能 |
|--------|------|
| [project-init](../project-init/) | インタビュー方式のプロジェクトスキャフォールディング — テンプレートではなく意思決定からCLAUDE.md、ROADMAP、.gitignore、.env.exampleを生成 |
| [setup](../setup/) | Claude Codeインフラ + エージェントチーム — rules、hooks、memory、ルーティング、エージェントインストールを一括で |

### 日常ワークフロー

| スキル | 機能 |
|--------|------|
| [scope](../scope/) | 実装前にIN/OUT/終了基準を定義。Quickモード（3つの質問）またはFullモード（レイヤードスペック） |
| [freeze](../freeze/) | 編集可能領域を宣言 — それ以外は凍結。実装中のスコープクリープを防止 |
| [goal-lock](../goal-lock/) | エージェント規律エンジン — 目標をロックし、PLAN→DO→VERIFY→FINALIZE→OUTPUTループを強制、13種類の成功偽装パターンを検出 |
| [pre-push](../pre-push/) | 必須pre-pushパイプライン — シークレットスキャン（12パターン）、ビルド/テスト、リント、並列AIコードレビュー。Critical/High発見時にpushをブロック |

### コードレビュー

| スキル | 機能 |
|--------|------|
| [code-autopsy](../code-autopsy/) | **更新 v7.2。** 12Q定量コードレビュー — 4軸スコア（Security/Stability/Robustness/Operability）、重要度アンカーテーブル、デプロイ判定（SHIP/FIX/RISKY/BLOCK）、Factuality Gate、CapCodeスコアgaming検出、CEF偽装エラー検出。スタンドアロンプロンプトとしてあらゆるLLMで使用可能 |

### 視点転換

| スキル | 機能 |
|--------|------|
| [stepback](../stepback/) | **更新。** ワンショット視点リセット — 1つの抽象的リフレーミング質問 + 3つのクイックチェック（スコープドリフト、副作用、より良いアプローチ）を10行以内で。作業中いつでも使用可能 |
| [next-action](../next-action/) | **新規。** ハンドオフ/git/lessons/STATEを読み込み、影響度順に次のアクション上位3件を提案。提案のみ、実行はしない。いつでも使用可能 |

### セッション管理

| スキル | 機能 |
|--------|------|
| [session-start](../session-start/) | 前セッションのハンドオフをロード、教訓レビュー、ヘルスチェック、優先アクション付き「準備完了」シグナル |
| [session-checkpoint](../session-checkpoint/) | compact前にセッションコンテキストを保存 — ハンドオフファイル、メモリ更新、教訓抽出、リフレクション（何がうまくいかなかったか、次にどう改善するか） |

### 品質

| スキル | 機能 |
|--------|------|
| [project-check](../project-check/) | 既存プロジェクトを4次元でスキャン：インフラ、セキュリティ、品質、ハーネス。深刻度順にギャップを整列 |
| [collab-audit](../collab-audit/) | 14セクションAIコラボレーション監査 — 実際の作業パターン（アンケートではなく）を分析し、行動プロファイル、盲点、成長方向を生成 |

### 運用

| スキル | 機能 |
|--------|------|
| [skill-ops](../skill-ops/) | **新規。** スキル/エージェント運用ハブ — スナップショット/ロールバック + 使用状況 + 呼び出し追跡の3モード |
| [project-overview](../project-overview/) | **新規。** 登録済みプロジェクトのセッションハンドオフから決定論的クロスプロジェクト状況マップを生成 |

### ガバナンス

| スキル | 機能 |
|--------|------|
| [full-audit](../full-audit/) | **新規。** 領域全体（コードベース/文書/スキル/メモリ/設定）の網羅的監査 — 決定論的スイープ+内容レビューの2層方式、偽陽性防止kill-test、永続カバレッジマップ |
| [integration-intake](../integration-intake/) | **新規。** 外部パターン（スキル/エージェント/ルール/プラグイン/MCP）導入の5項目スクリーニングゲート — 既存資産との重複チェック + 導入コンテンツのprovenance/インジェクションチェック |
| [clean-room](../clean-room/) | **新規。** 安全隣接リクエストを安全スコープに切り出し、完全に隔離されたfresh-contextサブエージェントが実行 — 敵対的検証パス + descope ledger |
| [eval-leakage-audit](../eval-leakage-audit/) | **新規。** eval/metric/holdoutが実際に独立した外部グラウンドトゥルースを確保しているか、循環的な自己確認かを18パターンのtaxonomyで監査。読み取り専用 |
| [doc-drift](../doc-drift/) | **新規。** Claude Codeがコンテキストに読み込むメモリ/ドキュメント(CLAUDE.md/MEMORY.md/skills/agents/commands)を古い主張・相互矛盾・危険/曖昧な表現で監査 — 優先度付き修正リストを生成 |

---

## ライフサイクルフロー

```
┌────────────────── セットアップ（1回）──────────────┐
│  /project-init  →  /setup                           │
└─────────────────────────────────────────────────────┘
         ↓
┌────────────────── 日常ループ ───────────────────────┐
│  /session-start                                      │
│       ↓                                              │
│  /scope → /freeze → /goal-lock → 作業                │
│       → /stepback (いつでも) → /code-autopsy → /pre-push│
│       ↓                                              │
│  /session-checkpoint                                 │
└──────────────────────────────────────────────────────┘
         ↓
┌────────────────── 随時 ────────────────────────────┐
│  /stepback         （視点リセット — いつでも）          │
│  /project-check    （ヘルス監査）                     │
│  /collab-audit     （行動診断）                       │
│  /integration-intake （外部資産導入前）               │
│  /full-audit       （領域全体の網羅監査）             │
│  /clean-room       （安全隣接スコープの分離）         │
│  /eval-leakage-audit （eval循環論理チェック）         │
│  /doc-drift        （読み込みコンテキストのドリフト監査）│
└─────────────────────────────────────────────────────┘
```

---

## インストール

### 方法A：スキルをコピー（最も簡単）

```bash
# 全スキルをインストール
git clone https://github.com/AlexZio00/sovereign-skills.git
cd sovereign-skills
for d in */; do [ -f "$d/SKILL.md" ] && cp -r "$d" ~/.claude/skills/; done

# または個別スキルをインストール
cp -r goal-lock ~/.claude/skills/
```

### 方法B：マーケットプレイス（sovereign-plugins）

このリポジトリはClaude Codeマーケットプレイスです。一度登録すればスキルを閲覧・インストールできます：

```bash
# Claude Codeでsovereign-pluginsマーケットプレイスを追加
# 設定 → プラグイン → マーケットプレイス追加 → https://github.com/AlexZio00/sovereign-skills.git
```

各スキルに個別の`.claude-plugin/plugin.json`メタデータも含まれています。

トリガーコマンド（例：`/goal-lock`）をClaude Codeで入力するとスキルが実行されます。

### 方法C: Codex / Cursor (npx)

各スキルに `agents/openai.yaml` が含まれています：

```bash
# Codex用スキルをインストール
npx skills add AlexZio00/sovereign-skills --skill goal-lock --agent codex -g -y

# Cursor用スキルをインストール
npx skills add AlexZio00/sovereign-skills --skill goal-lock --agent cursor -g -y

# Claude Code用インストール（方法Aの代替）
npx skills add AlexZio00/sovereign-skills --skill goal-lock --agent claude-code -g -y
```

SKILL.mdの内容は汎用です — マークダウン指示を読むあらゆるLLMで動作します。

### 要件

- **Claude Code**: CLI、デスクトップアプリ、またはウェブアプリ（[claude.ai/code](https://claude.ai/code)）
- **Codex**: OpenAI Codex（`npx skills` 対応）
- **Cursor**: Cursor IDE（スキルプラグイン対応）
- スキルディレクトリ：`~/.claude/skills/`（Claude Code）またはエージェント別パス
- `pre-push`にはPerlが必要（`scan_secrets.pl`同梱）

---

## v6.2の変更点

### 新規追加
- **stepback** — ワンショット視点リセット。抽象的なリフレーミング質問1つ（DeepMind step-backパターン）+ クイックチェック3つ（スコープドリフト、副作用、より良いアプローチ）を10行以内で生成。読み取り専用、エージェントなし、コードなし。実装中いつでも使用して、正しい問題を正しいレベルで解いているかを確認。出典：team-attention/hoyeon。

### 更新内容
- **code-autopsy** — メタ検出ゲート追加：スコアゲーミング検出用CapCodeシーリングメトリック、制約回避型偽装エラー検出用CEF。
- **collab-audit** — 13→14セクション。新セクション12：思考レベル軌跡（情報要求者→思考設計者の5段階モデル + 時間的変化追跡 + AI属性修正）。
- **goal-lock** — Ralph Wiggum早期完了検出（12番目の偽装パターン）+ VERIFY段階の検証トレーサビリティ（すべてのクレームが実際のツール呼び出しにトレース可能であること）追加。
- **session-checkpoint** — ハンドオフ明確性自己チェック追加（ハンドオフ作成後2つのアンカー質問）。
- **session-start** — コンテキスト腐敗防止（古いハンドオフエントリ用スライディングウィンドウ）。
- **pre-push** — 新規追加依存関係の3-IOCサプライチェーンチェック追加。
- **scope** — 禁忌フィールド追加（選択したアプローチが適切でない条件）。
- **freeze** — Thaw Protocol追加（正式なアンフリーズワークフロー + ブラストラジアス確認 + 3回の警告）。
- **project-init** — `.env.example`テンプレート拡張（OAuth、外部サービス、モニタリングセクション）+ セキュリティベースラインノート。
- **project-check** — スコアデルタ追跡追加（現在 vs 以前のスキャン結果比較）。
- **setup** — Tier 0違反テスト失敗向けRedesign Protocol追加（3オプションエスカレーション）。

---

## v6.1の変更点

### 新規追加
- **code-autopsy** — 12Q定量コードレビュープロンプト（Code Autopsy v7.0）。設計から監視可能性まで12の分析質問。4軸複合スコア（Security × 0.35 + Stability × 0.30 + Robustness × 0.20 + Operability × 0.15）。加重式付き重要度アンカーテーブル。CRITICALハードキャップ付きデプロイ判定。Factuality Gate（報告前の自己検証）。ファイル間影響分析。Quickモードおよびdiffモード。根拠：Google eng-practices、Johnson et al. 2019、Parnas 1972。あらゆるLLMでスタンドアロンプロンプトとして動作 — Claude Code専用ではありません。

---

## v6.0の変更点

### 新規追加
- **goal-lock** — PLAN→DO→VERIFY→FINALIZE→OUTPUTループを備えたエージェント規律エンジン。13種類の成功偽装パターン（テスト削除、mockラッピング、閾値緩和など）を検出。小さな変更用Quickモード（3フィールド）、それ以外はFullモード（7フィールド）。

### 統合
- `harness-init` + `team-init` → **setup** — インフラとエージェントチームが1度に
- `brief` + `adr` → **scope** — ADR機能が組み込まれたスコープ定義
- `retro` → **session-checkpoint** — レトロスペクティブは現在session-checkpoint内Phase 1.7 Reflexion

### 削除
- `token-audit` — 代わりに`npx ccusage`を直接使用するか、パターンからccusageスキルをビルド
- `adr`（スタンドアロン） — scopeに吸収されました
- `retro`（スタンドアロン） — session-checkpointに吸収されました

### アップグレード
- すべてのスキル：Dominant Variable、Key Assumptions、Error Recovery、Safety Layersを追加
- すべてのスキル：アクションタグ付きScope Boundary（[READ]/[WRITE]/[BASH]/[AGENT]）
- `session-checkpoint`：Memento CoT圧縮、Reflexion、Invocationロギング
- `pre-push`：大きなdiff決定論的バンドリング、Discard If条件
- `collab-audit`：アンチパターンフラグ、Key Assumptions

---

## エージェント設計パターンカバレッジ

この20個中17個のスキル（元々のライフサイクルセット、v6.4のガバナンス系新規スキル、v6.5の監査系新規スキル — v6.3の運用系新規スキルはまだマッピングされていません）は、25個の既知エージェント設計パターンのうち17個を実装しています（[Gulli 2026](https://books.google.com/books/about/Agentic_Design_Patterns.html?id=QqR20QEACAAJ), [Sairahul 2026](https://x.com/sairahul1/status/2069045570556383464)）：

| パターン | 実装スキル | 方法 |
|---------|-----------|------|
| **Sequential Pipeline** | session-start → scope → goal-lock → pre-push → checkpoint | 完全なライフサイクルチェーン |
| **Parallel Execution** | pre-push | 並列AIコードレビューエージェント |
| **Loop (Retry)** | goal-lock | VERIFY失敗 → PLAN再入場、上限あり |
| **Review & Critique** | pre-push, code-autopsy, full-audit, eval-leakage-audit | 独立したcode-reviewer + security-reviewer；12Q構造化レビュー；full-auditのPhase 2ファンアウトレビューパス；eval-leakage-auditはevalが独立したグラウンドトゥルースを確保しているか循環的自己確認かを批評 |
| **Iterative Refinement** | goal-lock | PLAN→DO→VERIFY→FINALIZE until DONE EVIDENCE通過 |
| **Coordinator/Router** | setup | エージェントルーティングルール生成 |
| **Plan-and-Execute** | goal-lock, scope | 実行前にレビュー可能な計画 |
| **ReAct** | project-check | 調査 → スコア → パス推奨 |
| **Reflexion** | session-checkpoint | Phase 1.7：失敗分析 → 次セッション用教訓 |
| **Human-in-the-Loop** | goal-lock, pre-push, integration-intake | STOP RULES、Critical/Highがpushをブロック；integration-intakeの導入前5項目スクリーニングゲート |
| **Custom Logic** | pre-push | 決定論的シークレットスキャン（Perl）+ AIレビュー |
| **Event-Driven** | session-start | セッション開始時にトリガー、前の状態をロード |
| **Guardrails/Safety** | goal-lock, clean-room | 13種類の成功偽装パターンを検出；clean-roomは安全隣接スコープをcarveして隔離されたサブエージェント実行に切り出す |
| **Memory Management** | session-checkpoint, doc-drift | ハンドオフファイル + メモリ更新 + 教訓抽出；doc-driftはコンテキストに読み込まれるメモリ/ドキュメントを古い主張・矛盾・危険な表現で監査 |
| **Goal Setting** | goal-lock | GOAL + DONE EVIDENCE入力シート |
| **Step-Back Abstraction** | stepback | DeepMind step-back：具体 → 抽象原則 |

---

## 設計原則

1. **テンプレートよりインタビュー** — 空の骨格ではなく、質問と意思決定から充実したコンテンツを生成
2. **信頼より検証** — 完了証拠は実行するもの、仮定するものではない。「通るはず」は検証ではない
3. **コードの前にスコープ** — ファイル変更前にIN/OUT/終了基準を定義。変更しないものは凍結
4. **正直な報告** — WORKING / PARTIAL / BROKENラベル。サイレント故障もモック偽装もなし
5. **セッション継続性** — ハンドオフで開始、チェックポイントで終了。コンテキストはセッションを超えて存続

---

## スキル間の連携関係

各スキルはfrontmatterの`see_also`（関連スキル）と`not_for`（誤用防止ガードレール）で関係を宣言します。主な関係：

| スキル | 連携先 | 関係 |
|--------|--------|------|
| `scope` | `goal-lock`, `freeze` | scopeは何を作るかを定義し、freezeは編集可能領域をロックし、goal-lockは実行ループを強制する |
| `freeze` | `scope`, `goal-lock` | freezeはscopeの計画とgoal-lockのループ強制をつなぐ手動ゾーンロック |
| `goal-lock` | `scope`, `freeze` | goal-lockはscope/freezeが定めた境界内で作動する実行時規律レイヤー |
| `stepback` | `next-action` | stepbackは方向確認（「正しい問題を解いているか」）、next-actionは行動推奨（「影響度順で次は何か」） |
| `next-action` | `session-start`, `stepback` | next-actionは現在の状態を読んで推奨し、session-startは前セッションの状態を復元する |
| `session-start` | `session-checkpoint` | ライフサイクルペア — セッションを開いて閉じる |
| `session-checkpoint` | `session-start`, `setup` | セッションを閉じ、setupは新規プロジェクトを開く |
| `code-autopsy` | `pre-push` | code-autopsyは深く踏み込んだオンデマンド12Qレビュー、pre-pushは毎push前に実行される高速自動パイプライン |
| `skill-ops` | `project-overview` | skill-opsはスキル/エージェントのライフサイクル（スナップショット/ロールバック/使用状況）を管理し、project-overviewは複数プロジェクトの状況を集計する |
| `integration-intake` | `full-audit` | integration-intakeは外部資産1件の導入可否をゲートし、full-auditは（既存のスキル/エージェント在庫を含め）領域全体のドリフト・ギャップを掃く |
| `full-audit` | `code-autopsy`, `project-check` | full-auditはより広い多領域の網羅監査＋永続カバレッジマップを残し、code-autopsyはファイル単位12Q、project-checkは4次元スコアのまま |
| `clean-room` | `goal-lock` | clean-roomは作業途中でスコープに安全隣接要素と安全な作業が混在した時に発火し、goal-lockはそれが割り込むPLAN→DO→VERIFYループ |
| `doc-drift` | `full-audit` | doc-driftはコンテキストに読み込まれるメモリ/ドキュメント（CLAUDE.md/MEMORY.md/skills/agents）のみをドリフト・矛盾で監査し、full-auditはカバレッジマップ付きで領域全体を掃く |
| `eval-leakage-audit` | `full-audit`, `code-autopsy` | eval-leakage-auditはeval/metric/holdoutが循環論理か（測定の完全性）を点検し、full-auditとcode-autopsyはevalの独立性ではなくコード/領域をレビューする |

図（矢印 = 「引き渡す」/「情報を与える」）：

```
setup ──> scope ──> freeze ──> goal-lock ──> pre-push
                                   │
                                stepback (いつでも、どの段階でも)
                                   │
session-start <──> session-checkpoint
                                   │
                            next-action (状態を読み推奨)
                                   │
    integration-intake / full-audit / clean-room / eval-leakage-audit / doc-drift
                     (オンデマンドガバナンス、どの段階でも)
```

---

## ライセンス

MIT — [LICENSE](../LICENSE)を参照。

## コントリビュート

IssueとPRを歓迎します。ライフサイクルに合うスキルを作成されたら、PRを開いてください。

## 連絡先

カスタムスキル開発のDMは[@AlexZio00](https://x.com/AlexZio00)まで。
