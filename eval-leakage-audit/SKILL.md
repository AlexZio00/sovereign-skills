...
2. **Report only patterns that fired** — don't mechanically list all 21; report only the ones actually backed by evidence. Violation → a laundry list dressed up as checklist completion, i.e. an unlabeled score dressed up as rigor.
...
| [READ] Apply the 21-pattern check and report only the ones that fired | Formally list patterns that didn't fire |
